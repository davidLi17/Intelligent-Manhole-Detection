          ...
          -->
			<el-amap
				...
				<el-amap-info-window
					...
					:offset="window.offset"></el-amap-info-window>
			</el-amap>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			initSearch() {
				...
				else pointMarker = this.UnRepairedCovers.concat(this.RepairedCovers);
				...
				pointMarker.forEach((item, index) => {
					...
					//动态生成详情页的url,比如: http://localhost:8080/#/detial/1
					let webUrl = window.location.href;
					let hashIndex = webUrl.indexOf("#");
					let baseUrl = webUrl.substring(
						0,
						hashIndex !== -1 ? hashIndex + 1 : webUrl.length
					);
					...
					let content =
						'<div style="background: white; width: 400px; padding: 10px;">\n' +
						'      <div style="width: 400px; height: 20px; line-height: 20px;">\n' +
						...
						"      <hr>\n" +
						'      <div style="float: left; height: 100px; margin: 5px 10px;">\n' +
						'        <img style="width:90px !important; height:100px !important;" src="' +
						predictedUrl +
						'">\n' +
						"      </div>\n" +
						...
						location +
						" </div>\n" +
						'        <div style="margin: 5px 10px;">经纬度：' +
						position +
						"</div>\n" +
						'        <div style="margin: 5px 10px; color: #20a0ff;"><a href="' +
						path +
						'">查看详情</a></div>\n' +
						"      </div>\n" +
						"    </div>";
