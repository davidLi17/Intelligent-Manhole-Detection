		...
		<div class="outerbox">
			<el-table
				...
				<el-table-column
					...
					label="维修人员"
					width="150"
					:filters="[{ text: '未分配', value: '未分配' }]"
					:filter-method="filterTag"
					filter-placement="bottom-end">
					<template slot-scope="scope">
						<el-tag
							:type="scope.row.repairer === '未分配' ? 'primary' : 'success'"
							@click="isdivided(scope.row)"
							id="but"
							disable-transitions
							>{{ scope.row.repairer }}</el-tag
						>
					</template>
				</el-table-column>
				<el-table-column
			...
			<el-dialog
				...
				<div class="Upagedivided">
					<el-pagination
						...
						:total="UarrayList.length"
						:page-size="UitemsPerPage">
					</el-pagination>
				</div>
			</el-dialog>
			...
			Todividede(id) {
				...
				getAllUser()
					.then((res) => {
						...
						this.UarrayList = this.UarrayList.filter(
							(obj) => obj.identity === "repairer"
						).map((obj) => {
							obj.identity = "维修人员";
							return obj;
						});
